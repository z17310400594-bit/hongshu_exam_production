...
| P8 | 小红书策略化生成 MVP | 是 | 否 | 前端可选策略/钩子，后端按策略生成卡片角色，执业药师真实烟测通过 |
...
把当前“固定卡片模板生成”升级为“小红书内容策略生成”，先解决内容生产里的两个核心矛盾：

- 写死：每篇都像同一个模板换皮；
- 写松：Dify 输出不可控，字段漂移，前端兜不住。

P8 不替代 P7 灰度。P8 是进入更大范围灰度前的产品能力增强，建议先完成 P8，再决定是否进入 P7。
...
```text
docs/implementation/P8_XHS_STRATEGY_MVP_PLAN.md
```
...
- 策略：倒计时救命型；
- 钩子：从今天开始备考；
- 结果：至少 6 张卡有效，前端无 `undefined`，引用非空。

## 12. 推荐下一步

当前 P0-P6 已完成，P5 真实链路已进入可调试状态。基于最新小红书样例分析，下一步建议先执行 P8，而不是直接进入 P7 灰度。

具体顺序：

1. P8：小红书策略化生成 MVP，先把生成内容形态做得可用。
2. P8-fix：根据执业药师真实烟测修一轮字段/展示/prompt。
3. P7：内部灰度试用。
4. P2-expand / P3-expand：继续扩真实数据和黄金集。

不要直接进入 P7。当前生成链路技术上能跑，但内容形态还偏工程模板；如果直接灰度，用户会先感受到“内容不像小红书”，而不是感受到 V2 知识库能力。
...
- `P8`：开始小红书策略化生成 MVP；
- `P8-plan-review`：先只审阅 P8 计划，不改代码；
- `P7`：跳过 P8，直接内部灰度；
- `修改计划`：先调整本文阶段或范围。
